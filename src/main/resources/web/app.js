// NexusAI Clean & ChatGPT-Grade Web Client
document.addEventListener('DOMContentLoaded', () => {
  ...
  function saveSessions() {
    // Only save valid sessions with messages
  ...
  async function handleSendMessage(overrideText = null) {
    ...
    // Get active session or create a new session
    ...
    saveSessions(); // Instantly adds & updates sidebar!
    ...
    try {
      ...
      if (res.ok) {
        ...
        renderMessageBubble('bot', 'Error communicating with AI Chatbot server.', 0.0, 'ERROR', 'en', false);
      }
    } catch (e) {
      typingBubble.remove();
      renderMessageBubble('bot', 'Network error or server is not responding.', 0.0, 'ERROR', 'en', false);
    } finally {
  ...
  function cleanTextForSpeech(rawText) {
    if (!rawText) return '';
    try {
      // Convert Markdown to clean text without tags
      const tempDiv = document.createElement('div');
      tempDiv.innerHTML = marked.parse(rawText);
      
      // Remove code blocks from audio
      ...
      // Remove symbols & formatting characters (clean natural flow)
  ...
  newChatBtn.addEventListener('click', () => {
    stopSpeaking();
    currentSessionId = null; // Fresh new conversation state
    localStorage.removeItem('nexus_current_session');
    messagesContainer.innerHTML = '';
    welcomeHero.style.display = 'flex';
    chatTextarea.value = '';
  ...
  teachForm.addEventListener('submit', async (e) => {
    ...
    try {
      const res = await fetch('/api/train', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ tag, category, patterns, responses })
      });
